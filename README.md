...
8. **[AI Engineering](AI%20Engineering/AI_Engineering_Interview_Prep.md)** — assumes general backend engineering (steps 1–5); cross-references REST API Design, Redis, and Transactions for patterns (retries, idempotency, caching) that apply directly to LLM-backed systems.
...
- **[Computer Science Glossary](Computer%20Science%20Fundamentals/Computer_Science_Glossary.md)** — a fast, 2–3-line-per-term reference covering 200 core CS/software-engineering terms, grouped by theme rather than alphabetically. Not a substitute for the interview-answer guides — it's the "what does this word mean" layer beneath them, with pointers into the guides that cover any given term in real depth.
...
Each of these three folders exists today only as a placeholder README stating its planned scope — there is no guide content in them yet. They're grouped here, separately from the completed topics above, so it's unambiguous which sections you can actually read right now.

- **[Design Patterns](Design%20Patterns/)** — classic GoF patterns and modern applications of them in real codebases.
- **[Frontend & Full-Stack](Frontend%20%26%20Full-Stack/)** — React/TypeScript fundamentals, browser/rendering performance, state management, and full-stack API integration concerns — the main gap for Full Stack Engineer loops.
