...
> **Target level:** Lead/Staff · **Baseline:** Apache Kafka 4.x (KRaft-only — ZooKeeper mode was fully removed in 4.0, March 2025); cooperative rebalancing assumes `CooperativeStickyAssignor` (KIP-429, Kafka 2.4+) as the classic-protocol default, which remains the client default in 4.x — Kafka 4.0 also shipped the next-generation consumer group protocol (KIP-848) as broker-default, but clients must still opt in via `group.protocol=consumer`, so everything this guide says about rebalancing describes the classic protocol unless a question says otherwise · **Last verified:** 2026-08-22 · **Prerequisites:** basic messaging/pub-sub concepts; helpful to have read [Transactions](Transactions_Interview_Prep.md) first for the dual-write-problem framing this file reuses

<!-- toc -->
## Table of Contents

- [PART 1: CONCEPTUAL FOUNDATIONS](#part-1-conceptual-foundations)
  - [1. Topics, Partitions, Offsets, Brokers, Producers, Consumers, Consumer Groups](#1-topics-partitions-offsets-brokers-producers-consumers-consumer-groups)
  - [2. What Ordering Does Kafka Guarantee?](#2-what-ordering-does-kafka-guarantee)
  - [3. How Message Keys Influence Correctness and Scalability](#3-how-message-keys-influence-correctness-and-scalability)
  - [4. What Happens During a Consumer-Group Rebalance?](#4-what-happens-during-a-consumer-group-rebalance)
  - [5. Eager vs. Cooperative Rebalancing](#5-eager-vs-cooperative-rebalancing)
  - [6. Static Consumer Memberships](#6-static-consumer-memberships)
  - [7. At-Most-Once, At-Least-Once, and Exactly-Once Delivery](#7-at-most-once-at-least-once-and-exactly-once-delivery)
  - [8. Why Consumers Must Remain Idempotent Even With Kafka Transactions](#8-why-consumers-must-remain-idempotent-even-with-kafka-transactions)
  - [9. Idempotent Producers vs. Transactional Producers](#9-idempotent-producers-vs-transactional-producers)
  - [10. `acks`, Retries, `min.insync.replicas`, and Replication Factor](#10-acks-retries-mininsyncreplicas-and-replication-factor)
  - [11. Trade-Offs of `acks=all`](#11-trade-offs-of-acksall)
  - [12. What Happens When a Producer Retry Changes Event Ordering?](#12-what-happens-when-a-producer-retry-changes-event-ordering)
  - [13. `max.in.flight.requests.per.connection` and Idempotence](#13-maxinflightrequestsperconnection-and-idempotence)
  - [14. When Should an Offset Be Committed?](#14-when-should-an-offset-be-committed)
  - [15. Crash After Database Commit But Before Offset Commit](#15-crash-after-database-commit-but-before-offset-commit)
  - [16. Automatic vs. Synchronous vs. Asynchronous Offset Commits](#16-automatic-vs-synchronous-vs-asynchronous-offset-commits)
  - [17. What Happens When Processing Exceeds `max.poll.interval.ms`?](#17-what-happens-when-processing-exceeds-maxpollintervalms)
  - [18. Handling Poison Messages](#18-handling-poison-messages)
  - [19. Retry Topics vs. Delayed Retry vs. Blocking Retry vs. DLQs — A Layered Strategy](#19-retry-topics-vs-delayed-retry-vs-blocking-retry-vs-dlqs--a-layered-strategy)
  - [20. What Metadata Must a DLQ Record Preserve?](#20-what-metadata-must-a-dlq-record-preserve)
  - [21. Designing Replay to Avoid Repeating Irreversible Side Effects](#21-designing-replay-to-avoid-repeating-irreversible-side-effects)
  - [22. Schema Compatibility: Avro, Protobuf, JSON Schema](#22-schema-compatibility-avro-protobuf-json-schema)
  - [23. Backward, Forward, and Full Compatibility](#23-backward-forward-and-full-compatibility)
  - [24. How to Remove a Field From an Event Schema](#24-how-to-remove-a-field-from-an-event-schema)
  - [25. How to Migrate to a New Partitioning Key](#25-how-to-migrate-to-a-new-partitioning-key)
  - [26. What Happens When the Number of Partitions Increases?](#26-what-happens-when-the-number-of-partitions-increases)
  - [27. How to Diagnose Consumer Lag](#27-how-to-diagnose-consumer-lag)
  - [28. Why Is Consumer Lag Alone an Incomplete Health Metric?](#28-why-is-consumer-lag-alone-an-incomplete-health-metric)
  - [29. Retention vs. Compaction](#29-retention-vs-compaction)
  - [30. What Guarantees Does a Compacted Topic Provide?](#30-what-guarantees-does-a-compacted-topic-provide)
  - [31. Reliable Database-to-Kafka Publication](#31-reliable-database-to-kafka-publication)
  - [32. Blue-Green Deployment of Kafka Consumers](#32-blue-green-deployment-of-kafka-consumers)
  - [33. Preventing Green From Processing Production Messages Before Cutover](#33-preventing-green-from-processing-production-messages-before-cutover)
  - [34. Same-Group vs. Shadow-Group vs. Separate-Topic Deployment Strategies](#34-same-group-vs-shadow-group-vs-separate-topic-deployment-strategies)
  - [35. Migrating Between Kafka Clusters](#35-migrating-between-kafka-clusters)
  - [36. How Backpressure and Overload Propagate Through an Event-Driven System](#36-how-backpressure-and-overload-propagate-through-an-event-driven-system)
  - [37. When Is Kafka the Wrong Messaging Technology?](#37-when-is-kafka-the-wrong-messaging-technology)
  - [38. Design: Ordering-Sensitive Payment Workflow Using Kafka](#38-design-ordering-sensitive-payment-workflow-using-kafka)
  - [39. Design: Replay Strategy for Correcting a Faulty Consumer Deployment](#39-design-replay-strategy-for-correcting-a-faulty-consumer-deployment)
  - [40. Metrics and Alerts for a Critical Kafka Pipeline](#40-metrics-and-alerts-for-a-critical-kafka-pipeline)
- [PART 2: SCENARIO-BASED QUESTIONS](#part-2-scenario-based-questions)
  - [S1. A consumer group's lag suddenly spikes on exactly one partition out of twelve, while the other eleven stay near zero. What do you investigate, and what's the likely root cause?](#s1-a-consumer-groups-lag-suddenly-spikes-on-exactly-one-partition-out-of-twelve-while-the-other-eleven-stay-near-zero-what-do-you-investigate-and-whats-the-likely-root-cause)
  - [S2. During a rolling deployment, you notice the consumer group triggers a rebalance for every single pod restart, even though only one pod restarts at a time and each comes back within seconds. How would you reduce this disruption?](#s2-during-a-rolling-deployment-you-notice-the-consumer-group-triggers-a-rebalance-for-every-single-pod-restart-even-though-only-one-pod-restarts-at-a-time-and-each-comes-back-within-seconds-how-would-you-reduce-this-disruption)
  - [S3. Your team needs to add a new required field to an event schema, but there are twelve downstream consumer teams you don't directly control, and you can't coordinate a synchronized deployment across all of them. How do you approach this?](#s3-your-team-needs-to-add-a-new-required-field-to-an-event-schema-but-there-are-twelve-downstream-consumer-teams-you-dont-directly-control-and-you-cant-coordinate-a-synchronized-deployment-across-all-of-them-how-do-you-approach-this)
  - [S4. A payment processing consumer occasionally double-charges customers. Investigation shows Kafka transactions are correctly configured and committing successfully. What's most likely going wrong, and how do you fix it?](#s4-a-payment-processing-consumer-occasionally-double-charges-customers-investigation-shows-kafka-transactions-are-correctly-configured-and-committing-successfully-whats-most-likely-going-wrong-and-how-do-you-fix-it)
  - [S5. You're asked to reduce Kafka infrastructure costs. One suggestion is to lower replication factor from 3 to 2 on all topics, including the payments pipeline. How do you evaluate this?](#s5-youre-asked-to-reduce-kafka-infrastructure-costs-one-suggestion-is-to-lower-replication-factor-from-3-to-2-on-all-topics-including-the-payments-pipeline-how-do-you-evaluate-this)
  - [S6. A consumer group has been stable for months, but after a recent Kafka Streams application update (adding new local state store logic), you start seeing the group rebalance every few minutes, seemingly at random, with no deployments or scaling events happening.](#s6-a-consumer-group-has-been-stable-for-months-but-after-a-recent-kafka-streams-application-update-adding-new-local-state-store-logic-you-start-seeing-the-group-rebalance-every-few-minutes-seemingly-at-random-with-no-deployments-or-scaling-events-happening)
  - [S7. Your company is migrating from an on-prem Kafka cluster to a cloud-managed Kafka service. Leadership wants zero downtime and zero data loss during the cutover. Walk through your approach.](#s7-your-company-is-migrating-from-an-on-prem-kafka-cluster-to-a-cloud-managed-kafka-service-leadership-wants-zero-downtime-and-zero-data-loss-during-the-cutover-walk-through-your-approach)
  - [S8. A teammate suggests using `acks=1` for the payments topic to "improve throughput since payments are important and we need speed." How do you respond?](#s8-a-teammate-suggests-using-acks1-for-the-payments-topic-to-improve-throughput-since-payments-are-important-and-we-need-speed-how-do-you-respond)
  - [S9. You need to remove a topic's compaction and switch it to a straightforward retention-based (time-limited) policy. What could go wrong if you do this without further planning, and what should you check first?](#s9-you-need-to-remove-a-topics-compaction-and-switch-it-to-a-straightforward-retention-based-time-limited-policy-what-could-go-wrong-if-you-do-this-without-further-planning-and-what-should-you-check-first)
  - [S10. A new engineer sets `enable.auto.commit=true` on a critical order-processing consumer "to keep things simple," and a few weeks later you discover a handful of orders were silently never processed after a consumer crash. Explain what happened and how you'd fix it going forward.](#s10-a-new-engineer-sets-enableautocommittrue-on-a-critical-order-processing-consumer-to-keep-things-simple-and-a-few-weeks-later-you-discover-a-handful-of-orders-were-silently-never-processed-after-a-consumer-crash-explain-what-happened-and-how-youd-fix-it-going-forward)
  - [S11. Your monitoring shows zero consumer lag on a critical pipeline, yet business stakeholders report that downstream dashboards are showing stale data for the last two hours. How is this possible, and what would you check?](#s11-your-monitoring-shows-zero-consumer-lag-on-a-critical-pipeline-yet-business-stakeholders-report-that-downstream-dashboards-are-showing-stale-data-for-the-last-two-hours-how-is-this-possible-and-what-would-you-check)
  - [S12. During a schema migration, one consumer team deploys their updated code a full week before the producer team finishes rolling out the new schema version. What compatibility mode should have been in place to prevent this from being a problem, and why?](#s12-during-a-schema-migration-one-consumer-team-deploys-their-updated-code-a-full-week-before-the-producer-team-finishes-rolling-out-the-new-schema-version-what-compatibility-mode-should-have-been-in-place-to-prevent-this-from-being-a-problem-and-why)
  - [S13. You're designing a new topic and need to decide the initial partition count. The team's instinct is to start with the current server capacity in mind (e.g., "we have 4 consumer pods, so 4 partitions"). What's wrong with this reasoning, and what would you recommend instead?](#s13-youre-designing-a-new-topic-and-need-to-decide-the-initial-partition-count-the-teams-instinct-is-to-start-with-the-current-server-capacity-in-mind-eg-we-have-4-consumer-pods-so-4-partitions-whats-wrong-with-this-reasoning-and-what-would-you-recommend-instead)
  - [S14. A DLQ has been quietly accumulating messages for three months. Nobody noticed until a customer complained about a transaction that never completed. What does this reveal about the original design, and how would you fix it?](#s14-a-dlq-has-been-quietly-accumulating-messages-for-three-months-nobody-noticed-until-a-customer-complained-about-a-transaction-that-never-completed-what-does-this-reveal-about-the-original-design-and-how-would-you-fix-it)
  - [S15. Leadership wants to know: "If Kafka guarantees ordering within a partition, why did we just have an incident where two updates to the same customer record were applied out of order?"](#s15-leadership-wants-to-know-if-kafka-guarantees-ordering-within-a-partition-why-did-we-just-have-an-incident-where-two-updates-to-the-same-customer-record-were-applied-out-of-order)
...
**Source:** [Apache Kafka Documentation — Introduction](https://kafka.apache.org/documentation/#intro_concepts)

---

### 2. What Ordering Does Kafka Guarantee?

Kafka's ordering guarantee is narrow and specific: **FIFO ordering within a single partition, and only within a single partition** — but even that partition-scoped guarantee depends on producer configuration. With `enable.idempotence=true` (the client default since Kafka 3.0), ordering within a partition is preserved across retries. Without idempotence, a producer with `max.in.flight.requests.per.connection > 1` can have a retried batch land after a later batch that succeeded on the first attempt, reordering records within the same partition (Q12) — so "strict FIFO" is the outcome of a specific, common, but not universal producer configuration, not an unconditional property of partitions themselves. There is no guarantee whatsoever about the relative order of records across different partitions, even within the same topic, even if they were produced at nearly the same instant.
...
When a producer sends a record with a key, Kafka's default partitioner applies a hash function to the key (`hash(key) % numPartitions`) to deterministically choose a partition. Every record with the same key lands in the same partition, *given a fixed partition count, a fixed partitioner, and a key serializer that always produces the same bytes for the same logical key* — three things that are easy to assume are constant and aren't: partition count changing (Q26) reshuffles the mapping going forward; swapping the partitioner implementation (or a custom `Partitioner`) changes the mapping outright; and a key serializer that isn't deterministic for logically-equal keys (e.g., a JSON serializer whose field order isn't stable) can route what your application considers "the same key" to different partitions even with nothing else changed. Given all three held constant, this gives you ordering *for that key* — this is what "correctness" means here: causally related events (e.g., all state transitions of a single order) stay in sequence.
...
**Source:** [Apache Kafka Documentation — Producer Configs](https://kafka.apache.org/documentation/#producerconfigs)
...
**Source:** [Apache Kafka Documentation — Consumer Configs](https://kafka.apache.org/documentation/#consumerconfigs)
...
**Why it matters in practice**: at scale (dozens of consumers, frequent rolling deploys), eager rebalancing's full-stop behavior means a routine pod restart during a deploy can pause the *entire* group's throughput for seconds, which compounds badly under CI/CD with frequent releases. Cooperative rebalancing was designed to make "sticky" assignments instead — most consumers keep exactly what they had, and only the affected partitions actually move — turning a group-wide pause into a much smaller, localized disruption.

**Source:** [KIP-429: Kafka Consumer Incremental Rebalance Protocol](https://cwiki.apache.org/confluence/display/KAFKA/KIP-429%3A+Kafka+Consumer+Incremental+Rebalance+Protocol), [Confluent — Incremental Cooperative Rebalancing in Apache Kafka](https://www.confluent.io/blog/incremental-cooperative-rebalancing-in-kafka/)

---

### 6. Static Consumer Memberships

By default, every time a consumer process restarts, the coordinator treats it as a brand-new group member — even if it's the same instance restarting seconds later after a deploy. That triggers an unnecessary rebalance for what is, functionally, a non-event.
...
- **Rolling deployments** — a rolling restart of N consumer pods, one at a time, would otherwise trigger N rebalances. Static membership eliminates the rebalance for the specific case of a restart that returns within `session.timeout.ms` under the same instance ID. It doesn't reduce rebalances to zero across the board — a consumer that fails to come back in time, a real scale-up/down (an actual membership-count change), or a subscribed topic's partition count changing (Q4) still triggers one. Static membership only removes the "same member, briefly restarting" case from the trigger list.
...
**Source:** [KIP-345: Introduce Static Membership Protocol](https://cwiki.apache.org/confluence/display/KAFKA/KIP-345%3A+Introduce+static+membership+protocol+to+reduce+consumer+rebalances)
...
- **At-least-once**: Offset is committed only *after* processing completes successfully. If a crash occurs between finishing the work and committing the offset, on restart the consumer will refetch and reprocess that same message — so duplicates are possible, and Kafka itself won't silently drop a message it hasn't yet gotten a committed offset past. That guarantee is about *Kafka's* delivery, though, not about the outcome for the business: it still depends on getting the commit-after-processing ordering actually right (auto-commit configured or timed incorrectly can silently regress this to at-most-once) and on the consumer's own processing logic not swallowing a failure (a caught exception that doesn't get rethrown, logged-and-ignored, or a partial write that isn't rolled back can still lose data at the application layer even though Kafka redelivered the message correctly). This is the most common production default, and it pushes the burden of correctness onto the consumer being idempotent (Q8).
...
**Source:** [Apache Kafka Documentation — Message Delivery Semantics](https://kafka.apache.org/documentation/#semantics)
...
**Source:** [Apache Kafka Documentation — Message Delivery Semantics](https://kafka.apache.org/documentation/#semantics), [Confluent — Exactly-once Semantics Is Possible: Here's How Apache Kafka Does It](https://www.confluent.io/blog/exactly-once-semantics-are-possible-heres-how-apache-kafka-does-it/)

---

### 9. Idempotent Producers vs. Transactional Producers

**Idempotent producer** (`enable.idempotence=true`): Solves a narrower problem — **duplicate writes caused by the producer's own retries**. Kafka assigns the producer a unique Producer ID (PID) and tags each batch with a per-partition sequence number. The broker tracks the expected next sequence number independently *for each partition* that PID writes to, so a producer can write to many partitions in the same session — the dedup/ordering guarantee just applies separately, per partition, not globally across them. If a network blip causes the producer to retry a batch that the broker actually already received and wrote, the broker recognizes the duplicate (PID, partition, sequence number) and discards it rather than writing it twice. Scope: this sequencing and deduplication holds *within* a single producer session (the PID is ephemeral — a producer restart gets a new one) and is tracked *per partition* — it protects against retry-induced duplication on each partition the producer writes to, but on its own it gives you **no atomicity across partitions or topics**: a batch of related writes to two different partitions can still succeed on one and fail on the other. That cross-partition atomicity is exactly what the transactional producer below adds.
...
**Source:** [KIP-98: Exactly Once Delivery and Transactional Messaging](https://cwiki.apache.org/confluence/display/KAFKA/KIP-98+-+Exactly+Once+Delivery+and+Transactional+Messaging), [Apache Kafka Documentation — Message Delivery Semantics](https://kafka.apache.org/documentation/#semantics)
...
**How they compose in a durable configuration**: a common production-grade setup is replication factor `3`, `min.insync.replicas=2`, `acks=all` — this tolerates the loss of any single broker *that was already fully in sync* without data loss, and without a durable write-unavailability window, as long as the remaining two replicas stay in the ISR. It's not an unconditional guarantee of zero write unavailability for every single-broker loss: if the lost broker was the partition leader, there's a brief unavailability window during leader election before the new leader starts accepting writes; if the ISR was already down to 2 replicas for some other reason (a prior, still-unresolved broker issue) when this third broker is lost, the ISR count drops below `min.insync.replicas` and writes fail outright until a replica catches back up. The configuration is a strong baseline, not a proof that a single-broker loss is always transparent to producers.

**Source:** [Apache Kafka Documentation — Producer Configs](https://kafka.apache.org/documentation/#producerconfigs), [Apache Kafka Documentation — Broker Configs](https://kafka.apache.org/documentation/#brokerconfigs)
...
The trade-off is structurally similar to — but not a literal instance of — the CAP theorem's consistency/availability tension: CAP formally describes what a distributed system can guarantee *during a network partition* between nodes, whereas the `acks=all` + `min.insync.replicas` trade-off is a synchronous-replication durability/availability choice that applies any time enough replicas are unavailable, partition or not (e.g., simple broker crashes with no network split at all). Calling it "classic CAP" overstates the connection; "a durability-vs-availability trade-off in the same family as CAP" is the more defensible framing. Either way, the right setting depends on whether your workload (e.g., financial transactions) values "never silently lose data" over "always accept writes, even if fewer replicas are healthy."
...
**Source:** [Confluent — Exactly-once Semantics Is Possible: Here's How Apache Kafka Does It](https://www.confluent.io/blog/exactly-once-semantics-are-possible-heres-how-apache-kafka-does-it/), [Chris Richardson — Transactional Outbox Pattern](https://microservices.io/patterns/data/transactional-outbox.html)
...
**Source:** [Confluent — Can Your Kafka Consumers Handle a Poison Pill?](https://www.confluent.io/blog/spring-kafka-can-your-kafka-consumers-handle-a-poison-pill/)
...
**Source:** [Spring for Apache Kafka — Non-Blocking Retries](https://docs.spring.io/spring-kafka/reference/retrytopic.html), [Confluent — Can Your Kafka Consumers Handle a Poison Pill?](https://www.confluent.io/blog/spring-kafka-can-your-kafka-consumers-handle-a-poison-pill/)
...
**Source:** [Spring for Apache Kafka — Non-Blocking Retries](https://docs.spring.io/spring-kafka/reference/retrytopic.html)
...
**Source:** [Chris Richardson — Idempotent Consumer](https://microservices.io/patterns/communication-style/idempotent-consumer.html), [Stripe API — Idempotent Requests](https://stripe.com/docs/api/idempotent_requests)
...
**Source:** [Confluent — Schema Evolution and Compatibility](https://docs.confluent.io/platform/current/schema-registry/fundamentals/schema-evolution.html)
...
**Source:** [Apache Kafka Documentation — Adding Partitions](https://kafka.apache.org/documentation/#basic_ops_add_partitions)
...
2. **Determine the lag trend, not just the current snapshot.** Is lag growing steadily (the consumer's processing throughput is actually below the incoming production rate — a capacity problem) or is it flat and non-zero (the consumer has stalled entirely — check for a crashed process, a rebalance loop, or a `poll()` that's stuck and never returning)?
...
**Source:** [Confluent Documentation — Monitor Consumer Lag](https://docs.confluent.io/platform/current/monitor/monitor-consumer-lag.html)
...
- **Retention** (`cleanup.policy=delete`, the default) deletes records once they exceed a configured age (`retention.ms`) or the log exceeds a configured size (`retention.bytes`), **regardless of the record's key**. This models a topic as a rolling window of events — appropriate when old events actually stop being relevant over time (e.g., raw clickstream data, application logs, telemetry).
...
**Source:** [Apache Kafka Documentation — Log Compaction](https://kafka.apache.org/documentation/#compaction)
...
**Source:** [Chris Richardson — Transactional Outbox Pattern](https://microservices.io/patterns/data/transactional-outbox.html)
...
**Source:** [Martin Fowler — BlueGreenDeployment](https://martinfowler.com/bliki/BlueGreenDeployment.html), [Apache Kafka Documentation — Consumer Configs](https://kafka.apache.org/documentation/#consumerconfigs)
...
1. **Establish cross-cluster replication**, most commonly via **MirrorMaker 2**, which replicates not just topic data but also — importantly — consumer group offsets and topic configuration from source to target cluster, and supports bidirectional replication if needed during a gradual transition.
...
**Source:** [Apache Kafka Documentation — Geo-Replication (MirrorMaker)](https://kafka.apache.org/documentation/#georeplication)
...
- **Low-throughput, simple task-queue needs.** If the actual requirement is "a basic job queue," Kafka's operational overhead (partition management, rebalancing, cluster operations) may not be justified relative to a simpler managed queue (e.g., SQS) for truly low-volume, low-complexity use cases.
...
- **Producer configuration**: `acks=all` for maximum durability, `enable.idempotence=true` to eliminate retry-induced duplicates, `min.insync.replicas=2` with a replication factor of 3 — giving the cluster a strong baseline for surviving a single broker failure without losing writes, and without a write-unavailability window as long as the ISR was already fully caught up when that broker was lost (Q10).
...
**Source:** [Apache Kafka Documentation — Producer Configs](https://kafka.apache.org/documentation/#producerconfigs), [Chris Richardson — Saga Pattern](https://microservices.io/patterns/data/saga.html)
...
4. **Ensure every downstream side effect triggered by replay is idempotent** (Q21). This needs to hold not just for the actually-incorrect records, but for the entire replayed window, since some of those records may have already been processed correctly the first time (only some subset was affected by the bug) — reprocessing them shouldn't double-apply already-correct effects.
...
**Source:** [Chris Richardson — Idempotent Consumer](https://microservices.io/patterns/communication-style/idempotent-consumer.html), [Apache Kafka Documentation — Consumer Configs](https://kafka.apache.org/documentation/#consumerconfigs)
...
**Source:** [Apache Kafka Documentation — Monitoring](https://kafka.apache.org/documentation/#monitoring)
...
The core issue is that a **required** field with no default breaks backward compatibility outright. Old-schema consumers reading new data would be fine — they'd just ignore the new field, satisfying forward compatibility — but you can't force every one of the twelve teams to instantly start populating a truly required field. If you make it required from day one, any consumer strictly validating against the new schema before all producers comply would break. The practical approach: introduce the field as **optional with a sensible default** first (satisfying full compatibility — Q23), let all producer teams migrate to populating it meaningfully over an agreed window, and monitor adoption (e.g., what percentage of messages now have a non-default value). Only once you've confirmed near-universal producer adoption should you consider whether the field actually needs to become "required" at the schema level at all — often a well-chosen default combined with monitoring is enough, since strict requirement is hard to enforce without central coordination anyway.
...
If Kafka transactions are actually configured and committing correctly, the exactly-once guarantee they provide is scoped to Kafka-internal state (records + offsets), not to the external payment gateway call (Q8). This is almost certainly a case of the **external side effect not being idempotent**, even though the Kafka-level plumbing is sound. The likely failure window: the consumer calls the payment gateway, the call succeeds, but the process crashes (or the transaction is aborted for some other reason) before the Kafka transaction commits. On retry, the same message is reprocessed and the payment gateway is called a second time for what Kafka considers "not yet successfully processed." The fix is to make the payment call itself idempotent, most robustly by passing a stable idempotency key (derived from the Kafka event's unique ID) through to the payment gateway's own idempotency-key mechanism if it supports one (most major payment processors do). That way, even a genuine duplicate call from your side gets deduplicated by the gateway itself, instead of relying solely on Kafka's internal guarantees, which were never designed to cover this external boundary.

**Source:** [Stripe API — Idempotent Requests](https://stripe.com/docs/api/idempotent_requests), [Confluent — Exactly-once Semantics Is Possible: Here's How Apache Kafka Does It](https://www.confluent.io/blog/exactly-once-semantics-are-possible-heres-how-apache-kafka-does-it/)
...
This needs to be evaluated per-topic based on durability requirements, not applied uniformly. Dropping from replication factor 3 to 2 does reduce storage cost roughly proportionally, but it also reduces fault tolerance. With replication factor 3 and `min.insync.replicas=2`, the cluster has headroom to lose a single, already-in-sync broker while still accepting writes (Q10). With replication factor 2, losing even a single broker either drops you to just one surviving replica (a durability risk, since a subsequent failure before recovery would mean data loss), or, if `min.insync.replicas` is also set to 2, means a single broker failure halts writes entirely (an availability risk) — you lose the buffer that made replication factor 3 resilient to a single-broker failure without losing either data or availability. For a payments pipeline specifically (Q38), where the earlier design explicitly chose replication factor 3 + `min.insync.replicas=2` for durability given financial stakes, this cost-saving change directly undermines the reasoning behind that original design decision. The right framing for the conversation: apply this cost reduction selectively to topics where the data is truly lower-stakes or easily reconstructible (e.g., high-volume, low-value telemetry), and explicitly exclude the payments pipeline — and any other topic where a single-broker failure causing potential data loss would be unacceptable — from this optimization.

**Source:** [Apache Kafka Documentation — Broker Configs](https://kafka.apache.org/documentation/#brokerconfigs)
...
This maps directly onto the cluster migration pattern (Q35). Key steps: set up MirrorMaker 2 to continuously replicate both topic data and consumer group offsets from the on-prem cluster to the new cloud cluster, and validate parity (message counts, checksums, spot-checks) before touching any production traffic. For the actual cutover, migrate consumers to the cloud cluster first, since reading from a validated, already-replicated target cluster is lower risk than reconfiguring the write path first — producers keep writing to on-prem during this phase, with MirrorMaker keeping the target cluster current. Once consumer-side stability on the cloud cluster is confirmed over a meaningful validation window, cut producers over to write directly to the cloud cluster. Offset translation (Q35) needs particular care here: the consumer groups resuming on the cloud cluster need to resume from the *correct logical position*, not an arbitrary one, which is exactly what MirrorMaker 2's offset-sync feature is built to handle. Run both clusters in parallel through the full validation window — with replication now potentially flowing both directions briefly, or simply monitoring cloud-side health independently — before fully decommissioning on-prem, treating the whole migration as gradual and reversible rather than a single hard cutover. That's what actually delivers on the "zero downtime, zero data loss" requirement, since a hard cutover with no validation window is where both risks tend to materialize.
...
Compacted topics (Q29, Q30) are typically used because consumers rely on "latest value per key, retained indefinitely" semantics (e.g., backing a KTable, or representing "current known state" for an entity). Switching to pure time-based retention means that once records age past `retention.ms`, they're deleted **regardless of whether they're still the current value for their key**. If a given key hasn't been updated recently (its last update happened, say, 8 months ago, older than your retention window), that key's current value simply vanishes from the topic once retention kicks in, even though nothing about that key's validity has actually changed. Any consumer relying on being able to read the topic from the beginning and end up with a complete, current picture of every key's latest value (the core guarantee compaction was providing) would now silently lose entries for any infrequently-updated key. Before making this change, you'd need to audit every downstream consumer to confirm none of them actually depend on that "complete current-state snapshot" guarantee. If any do, this change would require a more careful migration (e.g., a one-time export/snapshot of current compacted state elsewhere before the switch) rather than a simple policy flip.
...
Kafka's actual guarantee is scoped strictly to ordering *within a partition* (Q2), so an out-of-order incident for the same entity almost always traces back to one of a few root causes. The investigation should check each. **First**, was the entity actually keyed consistently — did both updates use the exact same key, guaranteeing they were routed to the same partition in the first place? A subtle key inconsistency (e.g., one service using `customerId` as a string and another formatting it inconsistently, or a case-sensitivity mismatch) would route logically-same-entity updates to *different* partitions, where Kafka's ordering guarantee simply doesn't apply. **Second**, did a partition count change (Q26) happen between the two updates? That shifts the key-to-partition mapping and could route what should be the same key's updates to different partitions on either side of the change. **Third**, was this a producer-side reordering issue from unacknowledged retries (Q12)? If idempotence wasn't enabled and `max.in.flight.requests.per.connection > 1`, a retried batch could genuinely land out of order even within the correct, same partition. **Fourth**, less likely but worth ruling out: was this actually a *consumer-side* processing-order issue rather than a Kafka delivery-order issue? If the consumer processes records concurrently across multiple threads without preserving per-key order in its own application logic, Kafka could have delivered them correctly in order while the application itself processed them out of order due to its own concurrency model. Presenting this to leadership as "the guarantee held, but here's the specific gap in how we used it" — rather than "Kafka failed" — is both more accurate and more useful for actually preventing recurrence.
