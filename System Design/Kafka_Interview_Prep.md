...
- **Rolling deployments** — a rolling restart of N consumer pods, one at a time, would otherwise trigger N rebalances; static membership can eliminate the rebalance triggered *specifically by a restart that returns within `session.timeout.ms` under the same instance ID*. It doesn't reduce rebalances to zero universally — a consumer that fails to come back in time, a genuine scale-up/down (a real membership-count change), or a subscribed topic's partition count changing (Q4) still triggers one; static membership only removes the "same member, briefly restarting" case from the trigger list.
